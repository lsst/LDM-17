...
algorithms that can be applied to a \code{Footprint} - is constructed from an 
\code{Exposure}; it is templated over \code{Exposure}'s pixel type. 
\code{MeasureSources}  acts as the driver behind the generalized 
\code{Footprint} iterator \code{FootprintFunctor} in applying its set of 
algorithms to a \code{Footprint} over the \code{Exposure}. The algorithms 
used can be specified by name in a \code{Policy} file.
